...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      stats: { totalUsers, totalProperties, activeListings, soldProperties },
    });
  } catch (error) {
    res.status(500).json({
      message: error.message,
...
export const getPendingSellers = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, count: pendingSellers.length, pendingSellers });
  } catch (error) {
    res.status(500).json({
      message: error.message,
    });
...
export const approveSeller = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Seller approved!.", seller });
  } catch (error) {
    res.status(500).json({
      message: error.message,
    });
