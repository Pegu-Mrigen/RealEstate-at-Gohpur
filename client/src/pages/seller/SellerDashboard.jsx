...
const SellerDashboard = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        const [ statsRes, propsRes, inqRes ] = await Promise.all([
          ...
          axios.get(`${API_URL}/inquiry/seller`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          }),
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Are you sure to delete this listing? This action cann't be undone."
      )
    )
      return;
    try {
  ...
  const handleStatusUpdate = async (id, currentStatus) => {
    const newStatus = currentStatus === "sold" ? "sold" : "sale";
    try {
      await axios.patch(
          ...
          <div className={s.tipsList}>
            ...
            <div className={s.tipCardMarket}>
              ...
              <p className={s.tipTextMarket}>
                Properties in your area selling fast. Your prices are
                competative.
              </p>
            </div>
          </div>
